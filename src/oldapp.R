...
ui <- shinyUI(navbarPage(title = "CNV Incidence Calculator v2.1", 
...
observeEvent(input$load_data,{
    ...
    conn <<- dbConnect(drv = RPostgres::Postgres(),
                      ...
                      password='password', 
                      port=5432)
    dbExecute(conn, "SET work_mem = '1.5GB';")
...
observeEvent(input$load_single, {
  ...
  output$igene_hist <- renderPlot({
    ...
    if (input$alteration_single=='CNV'){
      ...
      snpdat <- tbl(conn, 'snp_main') %>% #flatten multiple snps in a single patient
  ...
  output$top_cnv <- DT::renderDataTable({
    ...
    if (input$zygosity_single=='False'){
      subset <- tdat %>% 
        ...
        mutate(nsamp=sum(freq), rel_incidence=freq/nsamp) %>% 
        filter(alteration!=0) %>% 
        mutate(CNV=case_when(alteration<0~'Deletion', alteration>0~'Duplication')) %>% 
        select(-alteration) %>%
        ungroup() %>% 
        group_by(cancer, CNV) %>% 
        mutate(rel_incidence=sum(rel_incidence)) %>% 
        relocate(cancer, CNV) 
